...
def process_folder(working_dir, name, output_dir, removeFolder=False, logger=default_logger):
    ...
    # Filter out hidden files (e.g., macOS ._ files)
    txt_list = [f for f in txt_list if not basename(f).startswith('.')]
    
    if not txt_list:
        return

    # Move TOC files to the end (e.g. *-toc.txt or *_toc.txt)
    tocs = [f for f in txt_list if '-toc' in basename(f).lower() or '_toc' in basename(f).lower()]
    volumes = [f for f in txt_list if f not in tocs]
    
    txt_list = volumes + tocs
